      ...
      for (var key in SQLBlocks.Msg.DROPDOWN.GROUPFUNCTIONS) {
        ...
        if (n !== -1) {
          if (column.lastIndexOf(")") !== -1) {
            ...
            tableName = groupfunction
              .substring(g.length + 1, column.indexOf(")") - 1)
              .split(".")[0]
              .trim();
          } else {
            groupfunction = column.substring(n, n + g.length + 1).trim();
            tableName = groupfunction
              .substring(g.length + 1, column.length - 1)
              .split(".")[0]
              .trim();
          }

          if (
            !(tableName in tablesArray) &&
            !(
              tableName.length === 0 || /^\s*$/.test(tableName)
            ) /* TODO: Abfrage in Funktion auslagern! */
          ) {
